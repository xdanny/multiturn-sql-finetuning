"""Build semantic-layer SQL finetuning rows from curated synthetic fixtures."""
...
ARTIFACT_TYPE = "semantic_layer_training_rows"
BENCHMARK = "synthetic_semantic_layer"
DEFAULT_OUTPUT = Path("docs/data_artifacts/semantic_layer_training_rows.jsonl")
DEFAULT_SUMMARY_OUTPUT = Path("docs/data_artifacts/semantic_layer_training_rows_summary.json")
DEFAULT_MANIFEST_OUTPUT = Path("docs/data_artifacts/semantic_layer_training_rows.manifest.json")
...
def _semantic_layer_system_prompt() -> str:
    return (
        "You write analytical SQL for multi-turn data analysis. Use the visible schema, "
        "conversation, and semantic model. Respond with only SQL."
    )


def _semantic_layer_user_prompt(fixture: dict[str, Any]) -> str:
    visible = fixture["prompt_visible_input"]
    semantic_model = json.dumps(visible["semantic_model"], sort_keys=True)
    prompt = (
        "You are learning semantic-layer-aware SQL generation for multi-turn data analysis.\n"
    ...
    for marker in FORBIDDEN_PROMPT_MARKERS:
        if marker in prompt:
            raise ValueError(f"semantic-layer prompt leaked forbidden marker: {marker}")
    return prompt


def build_semantic_layer_training_rows(fixtures: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        rows.append(
            {
                ...
                "messages": [
                    {"role": "system", "content": _semantic_layer_system_prompt()},
                    {"role": "user", "content": _semantic_layer_user_prompt(fixture)},
...
def summarize_semantic_layer_training_rows(rows: list[dict[str, Any]]) -> dict[str, Any]:
...
def write_semantic_layer_training_artifacts(
    *,
    ...
    rows = build_semantic_layer_training_rows(fixtures)
    summary = summarize_semantic_layer_training_rows(rows)
...
def main() -> int:
    ...
    print(f"Wrote {manifest['row_count']} semantic-layer finetuning rows to {args.output}")
