"""Build direct-SQL control rows for the synthetic behavior-recovery comparison."""
...
DEFAULT_OUTPUT = Path("docs/data_artifacts/behavior_recovery_direct_sql_training_rows.jsonl")
DEFAULT_SUMMARY_OUTPUT = Path(
    "docs/data_artifacts/behavior_recovery_direct_sql_training_rows_summary.json"
)
DEFAULT_MANIFEST_OUTPUT = Path(
    "docs/data_artifacts/behavior_recovery_direct_sql_training_rows.manifest.json"
)
...
def _direct_sql_system_prompt() -> str:
    return (
        ...
        "conversation, and execution feedback. Respond with only SQL."
    )
...
def _direct_sql_user_prompt(fixture: dict[str, Any]) -> str:
    ...
    return (
        "You are learning a direct-SQL control for behavior recovery.\n"
...
def build_behavior_recovery_direct_sql_training_rows(fixtures: list[dict[str, Any]]) -> list[dict[str, Any]]:
...
def summarize_behavior_recovery_direct_sql_training_rows(rows: list[dict[str, Any]]) -> dict[str, Any]:
...
def write_behavior_recovery_direct_sql_training_artifacts(
    *,
...
def main() -> int:
    ...
    manifest = write_behavior_recovery_direct_sql_training_artifacts(
    ...
    print(
        "Wrote "
        f"{manifest['row_count']} behavior-recovery direct-SQL control rows to {args.output}"
    )
