...
def prune_semantic_model_context(semantic_model: str, relevant_tables: list[str]) -> str:
    """Keep only semantic-model cube blocks relevant to the current gold SQL."""

    relevant = {table.lower() for table in relevant_tables}
    if not semantic_model or not relevant:
        return semantic_model

    blocks: list[list[str]] = []
    current: list[str] = []
    for line in semantic_model.splitlines():
    ...
    kept = []
    cube_pattern = re.compile(r"^- Cube\s+([^\s(]+)", re.IGNORECASE)
    for block in blocks:
        match = cube_pattern.match(block[0])
        cube_name = match.group(1).lower() if match else ""
        if cube_name in relevant:
            kept.append("\n".join(block))
    return "\n".join(kept) if kept else semantic_model


def planning_hint_from_labels(labels: dict[str, Any]) -> str:
    """Format compact oracle SQL planning guidance for a user turn."""

    if not labels.get("parseable"):
        return (
            "Oracle SQL planning hints "
            "(derived from reference SQL; diagnostic only):\n"
            "Gold SQL labels unavailable; rely on the schema and history."
        )

    skeleton = labels.get("query_skeleton") or {}
    projection = labels.get("projection_shape") or {}
    ...
    lines = [
        ...
        f"Relevant tables: {', '.join(labels.get('relevant_tables') or []) or 'none'}",
        f"Relevant columns: {', '.join(labels.get('relevant_columns') or []) or 'none'}",
        f"Join path: {'; '.join(labels.get('join_path') or []) or 'none'}",
        f"Query skeleton: {', '.join(skeleton_flags) or 'select'}",
        (
            "Projection shape: "
            f"{projection.get('selected_count', 0)} selected expression(s) in this order: "
            f"{'; '.join(projection.get('selected_expressions') or []) or 'none'}"
        ),
    ]
    if projection.get("aggregations"):
        lines.append(f"Aggregation outputs: {', '.join(projection['aggregations'])}")
    if projection.get("group_by"):
        lines.append(f"Group by: {', '.join(projection['group_by'])}")
    if projection.get("order_by"):
        lines.append(f"Order by: {projection['order_by']}")
    if projection.get("limit"):
        lines.append(f"Limit: {projection['limit']}")
    duplicate_note = "preserve duplicate rows" if projection.get("preserve_duplicates") else "deduplicate rows"
