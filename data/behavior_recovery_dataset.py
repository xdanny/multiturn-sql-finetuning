"""Build behavior-recovery finetuning rows from curated synthetic fixtures."""
...
DEFAULT_OUTPUT = Path("docs/data_artifacts/behavior_recovery_training_rows.jsonl")
...
def _behavior_recovery_system_prompt() -> str:
    return (
        "You write analytical SQL for multi-turn data analysis. Use the visible schema, "
        "conversation, execution feedback, and allowed semantic context. Respond with only SQL."
    )


def _behavior_recovery_user_prompt(fixture: dict[str, Any]) -> str:
    visible = fixture["prompt_visible_input"]
    semantic_model = json.dumps(visible["semantic_model"], sort_keys=True)
    return (
        "You are learning behavior recovery for multi-turn SQL analysis.\n"
...
def build_behavior_recovery_training_rows(fixtures: list[dict[str, Any]]) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    for fixture in fixtures:
        if "behavior_recovery" not in fixture.get("training_targets", []):
            continue
        rows.append(
            {
                ...
                "messages": [
                    {"role": "system", "content": _behavior_recovery_system_prompt()},
                    {"role": "user", "content": _behavior_recovery_user_prompt(fixture)},
...
def summarize_behavior_recovery_training_rows(rows: list[dict[str, Any]]) -> dict[str, Any]:
...
def write_behavior_recovery_training_artifacts(
    *,
...
def main() -> int:
    ...
    manifest = write_behavior_recovery_training_artifacts(
    ...
    print(f"Wrote {manifest['row_count']} behavior-recovery finetuning rows to {args.output}")
