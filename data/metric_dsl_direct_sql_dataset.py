"""Build direct-SQL control rows for the Stage 4 metric-heavy comparison."""
...
ARTIFACT_TYPE = "metric_dsl_direct_sql_training_rows"
DEFAULT_OUTPUT = Path("docs/data_artifacts/metric_dsl_direct_sql_training_rows.jsonl")
DEFAULT_SUMMARY_OUTPUT = Path("docs/data_artifacts/metric_dsl_direct_sql_training_rows_summary.json")
DEFAULT_MANIFEST_OUTPUT = Path("docs/data_artifacts/metric_dsl_direct_sql_training_rows.manifest.json")


def _direct_sql_system_prompt() -> str:
    return (
        "You write analytical SQL for multi-turn data analysis. Use the visible schema and "
        "business rules, and respond with only SQL."
    )
...
def _direct_sql_user_prompt(fixture: dict[str, Any]) -> str:
    ...
    return (
        "You are learning a direct-SQL control for metric-heavy analysis.\n"
...
def build_metric_dsl_direct_sql_training_rows(fixtures: list[dict[str, Any]]) -> list[dict[str, Any]]:
...
def summarize_metric_dsl_direct_sql_training_rows(rows: list[dict[str, Any]]) -> dict[str, Any]:
...
def write_metric_dsl_direct_sql_training_artifacts(
    *,
    ...
    rows = build_metric_dsl_direct_sql_training_rows(fixtures)
    summary = summarize_metric_dsl_direct_sql_training_rows(rows)
...
def main() -> int:
    ...
    print(f"Wrote {manifest['row_count']} metric-DSL direct-SQL control rows to {args.output}")
