...
def test_summarize_planner_readiness_blocks_projection_order_mismatch() -> None:
    rows = [
        _row(
            macro=0.90,
            table=0.95,
    ...
    assert summary["selected_expression_order_mismatch_count"] == 3
    assert summary["mean_scores"]["selected_expression_order_match"] == 0.875
    assert summary["promotion_ready"] is False
